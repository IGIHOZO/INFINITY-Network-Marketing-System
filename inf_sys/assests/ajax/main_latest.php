					...
					if ($amnt>=8000) {
						...
						if ($cnt_sel_fees==1) {
							...
							if ($upd_balance_ok) {
								...
								$ins_tr_rec = $conn->prepare("INSERT INTO Encashment(encash_net_income,encash_tax,encash_sys_fees,encash_before,encash_remain,encash_gross_income,encash_status) VALUES(?,?,?,?,?,?,?)");
								$ins_tr_rec->bindValue(1,$net_income);
								$ins_tr_rec->bindValue(2,$tax);
								$ins_tr_rec->bindValue(3,$sys_fees);
								$ins_tr_rec->bindValue(4,$before);
								$ins_tr_rec->bindValue(5,$remain);
								$ins_tr_rec->bindValue(6,$gross_income);
								$ins_tr_rec->bindValue(7,'E');
								...
								if ($ins_tr_rec_ok) {
									...
									echo "<br> Visit your <i>Encashement History<i/> to view more Details.";
								}else{
									echo "Your Encashment did not complete successfully. Please Contact System-adminstrator as soon as possible.";
								}
