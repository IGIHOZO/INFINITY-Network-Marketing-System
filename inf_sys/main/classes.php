...
function pending_encashment_list(){
	...
	if ($sel_enc_lst->rowCount()>0) {
		echo "<table class='encashment-tbl'><thead class='th_1'>";
		echo "<th colspan=13>Pending encashment list</th>";
		echo "</thead>";
		echo "<thead class='th_2'>";
...
//================================================================================================== DISPLAY TODAY'S REGISTERED Accounts



function today_registered_accounts(){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT accounts.*,member.* FROM accounts,member WHERE accounts.account_member=member.member_id AND LEFT(accounts.account_registration_date,10)='".date("Y-m-d")."'");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		echo "<table><thead>";
		echo "<th colspan=13>Today's registered accounts  (".date('Y-m-d').")</th>";
		echo "</thead><thead>";
		echo "<th>#</th>";
		echo "<th>Date</th>";
		echo "<th>Names</th>";
		...
		echo "</thead><tbody>";
		$cnt = 1;
		while ($ft_sel_acc_td = $sel_acc_td->fetch(PDO::FETCH_ASSOC)) {
			switch ($ft_sel_acc_td['account_status']) {
				case 'NY':
					$acc_stt = "<span style='color:#4a1d79;'>Pending</span>";
					break;
				case 'E':
					$acc_stt = "<span style='color:green;'>Confirmed</span>";
					break;
				
				default:
					$acc_stt = "<span style='color:red;'>---</span>";
					break;
			}
			echo "<tr>";

			echo "<td>".$cnt.". </td>";
			echo "<td>".$ft_sel_acc_td['account_registration_date']."</td>";
			echo "<td>".strtoupper($ft_sel_acc_td['member_fname'])." ".ucfirst($ft_sel_acc_td['member_lname'])."</td>";
			echo "<td>".$ft_sel_acc_td['account_pincode']."</td>";
			echo "<td>".$acc_stt."</td>";
		...
		echo "<center><h3>No accounts registered today.</h3></center>";
	}
}

//=============================================================== DISPLAY ACCOUNTS REGISTERED ON A CERTAIN DATE



function certain_day_registered_accounts($date){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT accounts.*,member.* FROM accounts,member WHERE accounts.account_member=member.member_id AND LEFT(accounts.account_registration_date,10)='$date'");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		echo "<table><thead>";
		echo "<th colspan=13>Accounts registered on $date</th>";
		echo "</thead><thead>";
		echo "<th>#</th>";
		echo "<th>Date</th>";
		echo "<th>Names</th>";
		...
		echo "<center><h3>No accounts registered $date.</h3></center>";
	}
}

//================================================================================ RETURN NUMBER OF ALL ACCOUNTS

function num_all_accounts(){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT COUNT(accounts.account_id) AS all_acnt FROM accounts");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		return $ft_sel_acc_td=$sel_acc_td->fetch(PDO::FETCH_ASSOC)['all_acnt'];
	}else{
		return 0;
	}
}

//================================================================================ RETURN NUMBER OF ALL PENDING

function num_all_pending_accounts(){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT COUNT(accounts.account_id) AS all_acnt FROM accounts WHERE accounts.account_status='NY'");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		return $ft_sel_acc_td=$sel_acc_td->fetch(PDO::FETCH_ASSOC)['all_acnt'];
	}else{
...
//================================================================================ RETURN NUMBER OF ALL CONFIRMED

function num_all_comfirmed_accounts(){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT COUNT(accounts.account_id) AS all_acnt FROM accounts WHERE accounts.account_status='E'");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		return $ft_sel_acc_td=$sel_acc_td->fetch(PDO::FETCH_ASSOC)['all_acnt'];
	}else{
...
//================================================================================ RETURN NUMBER OF TODAY'S REGISTERED ACCOUNTS

function num_today_accounts(){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT accounts.*,COUNT(accounts.account_pincode) AS ac_nmbr,member.* FROM accounts,member WHERE accounts.account_member=member.member_id AND LEFT(accounts.account_registration_date,10)='".date("Y-m-d")."'");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		return $ft_sel_acc_td=$sel_acc_td->fetch(PDO::FETCH_ASSOC)['ac_nmbr'];
	}else{
		return 0;
	}
}
...
function num_certain_day_accounts($date){
	$conn = parent::connect();
	$sel_acc_td = $conn->prepare("SELECT accounts.*,COUNT(accounts.account_pincode) AS ac_nmbr,member.* FROM accounts,member WHERE accounts.account_member=member.member_id AND LEFT(accounts.account_registration_date,10)='$date'");
	$sel_acc_td->execute();
	if ($sel_acc_td->rowCount()>0) {
		return $ft_sel_acc_td=$sel_acc_td->fetch(PDO::FETCH_ASSOC)['ac_nmbr'];
	}else{
...
//============================================================================= TOTAL SYSTEM ENCASHED BALANCE

function ttl_sys_encash_gross(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(encashment.encash_net_income) AS sys_enc_one FROM encashment");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_enc_one'];
	}else{
		return 0;
	}
}
...
//============================================================================= TOTAL SYSTEM EARNED BY AGENTS ENCASHEMENTS

function ttl_sys_earned_encash(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(encashment.encash_gross_income) AS sys_en_aern FROM encashment");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_en_aern'];
	}else{
		return 0;
	}
}

//============================================================================= TOTAL SYSTEM AVAILABLE BALANCE (Balance_all)

function ttl_sys_balance_available(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_total) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
		return 0;
	}
}

//============================================================================= TOTAL SYSTEM INDEIRECT BALANCE 

function ttl_sys_upgrades_available(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_upgrade) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
//============================================================================= TOTAL SYSTEM MATCHING BALANCE 

function ttl_sys_matching_available(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_matching) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
//============================================================================= TOTAL SYSTEM DAILY SPONSOR BONUS BALANCE (daily sponsoring)

function ttl_sys_daily_sponsor_bonus_available(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_daily_sponsor) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
//============================================================================= TOTAL SYSTEM ACCOUNT BONUS BALANCE (direct acc_sponsoring)

function ttl_sys_direct_sponsor_bonus_available(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_direct_sponsors) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
//============================================================================= TOTAL SYSTEM COMMISSION BONUS BALANCE 

function ttl_sys_commission_balance_available(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_commission) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
//============================================================================= TOTAL SYSTEM ALL BALANCE 

function ttl_sys_balance_all(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(balance.balance_all) AS sys_bal_all FROM balance");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
function general_profit_or_loss(){
	$all_money = self::num_all_accounts()*25000;
	$all_all_balance = self::ttl_sys_balance_all();
	return $all_money-$all_all_balance;
}


//============================================================================= TOTAL SYSTEM ENCASHMENT TAX+SYSTEM_FEES (17%) 

function all_sys_tax_17_balance(){
	$conn = parent::connect();
	$sel_en_gr = $conn->prepare("SELECT SUM(encashment.encash_tax) AS sys_bal_all FROM encashment");
	$sel_en_gr->execute();
	if ($sel_en_gr->rowCount()>0) {
		return $sel_en_gr->fetch(PDO::FETCH_ASSOC)['sys_bal_all'];
	}else{
...
//============================================================================= TOTAL SYSTEM ENCASHMENT TAX ONLY (15%) 

function all_sys_tax_15_balance(){
	$all_tax_fees = self::all_sys_tax_17_balance();
	$onlt_tax = $all_tax_fees*15/17;
	return $onlt_tax;
}
//============================================================================= TOTAL SYSTEM ENCASHMENT SYSTEM FEES (2%) 

function all_sys_tax_2_sys_fees_balance(){
	$all_tax_fees = self::all_sys_tax_17_balance();
	$onlt_tax = $all_tax_fees*2/17;
	return $onlt_tax;
}
